...
async function getRouter(env: Env): Promise<{ router: EdgeRouter; storage: StorageAdapter }> {
  ...
  // Provider credentials are encrypted at rest when MASTER_KEY is configured.
  // Existing plaintext rows are migrated in-place before they can reach routing.
  let protectedStorage: StorageAdapter = rawStorage;
  if (env.MASTER_KEY) {
    protectedStorage = await SecretStorageAdapter.create(rawStorage, env.MASTER_KEY);
    for (const provider of await rawStorage.getProviders()) {
      if (
        provider.apiKey ||
        ...
        await protectedStorage.saveProvider(provider);
      }
    }
  }
  const storage = new CachedStorageAdapter(protectedStorage, 60000);
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (path === "/api/providers" && request.method === "GET") {
      ...
      return Response.json(
        { providers: providers.map(redactProviderSecrets), total: providers.length },
        { headers: { "Access-Control-Allow-Origin": "*" } }
      );
    }
